...
const Videos = () => {
  ...
  const [lectureTitle , setLectureTitle] = useState(videosArr[0].title);
  const videoChange= (url,title) => {
      setVideoSrc(url);
  ...
  return (
    <Stack direction={['column', 'column', 'row']} h={'100vh'}>
      ...
      <VStack w={['full', 'full', 'xl']} alignItems={'stretch'} p={8} spacing={8} overflowY={'auto'}>
        <Heading textAlign={'center'}>Lectures</Heading>
        {
          videosArr.map((item, index)=>(
            // <Button variant={'ghost'} colorScheme='facebook' onClick={()=>setVideoSrc(item)} key={index}>Lecture {index +1}</Button>
            <Button variant={'ghost'} colorScheme='facebook' onClick={()=>videoChange(item.url, item.title)} key={index}>Lecture {index +1}</Button>
          ))
        }
      </VStack>
    </Stack>
